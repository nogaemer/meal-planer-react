 ...
 * @param {string} props.mealId - Unique meal identifier for navigation
 * @param {string} props.title - Meal name displayed as heading
 * @param {string} props.description - Brief meal description (single line truncated)
 * @param {string} props.imageUrl - URL for meal cover image
 * @param {number} props.rating - Average meal rating (displayed with 1 decimal)
 * @param {number} props.prepTime - Preparation time in minutes
 * @param {(el: HTMLDivElement | null) => void} props.imgRef - Callback ref for image element (lazy loading)
 * @param {() => void} props.handleImageClick - Handler for card/image click events
 * @param {boolean} [props.priority=false] - Whether to load image with high priority (for above-fold cards)
 *
...
const MealCard: React.FC<MealCardProps> = (
    ...
    <div className="w-full max-w-md mx-auto h-fit bg-card rounded-3xl overflow-hidden shadow-md flex flex-col"
        ...
        {imageUrl && <img src={imageUrl} alt={title} className="h-40 object-cover cursor-pointer" ref={imgRef}
                          fetchPriority={priority ? "high" : "auto"} loading={priority ? "eager" : "lazy"}/>}
 ...
 *
 * @param {MealCardInfoProps} props - Component props
 * @param {string} props.label - Descriptive label text
 * @param {string|number} props.value - Display value (rating score, time, etc.)
 * @param {string} [props.iconWrapperClass=""] - Optional CSS classes for icon wrapper
